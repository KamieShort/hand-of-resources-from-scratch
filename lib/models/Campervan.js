...
module.exports = class Campervan {
  ...
  static async updateById(id, attrs) {
    const campervan = await Campervan.getById(id);
    const { make, model, builder } = { ...campervan, ...attrs };
    const { rows } = await pool.query(
      `UPDATE campervans
          SET make=$2, model=$3, builder=$4
          WHERE id=$1 RETURNING *`,
      [id, make, model, builder]
    );
    return new Campervan(rows[0]);
  }
  ...
  static async delete(id) {
    const { rows } = await pool.query(
      'DELETE FROM campervans WHERE id = $1 RETURNING *',
      [id]
    );
    return new Campervan(rows[0]);
  }
